...
| POST | `/domain/decision` | 执行云成本与 FinOps 管理平台专属领域规则 |
...
| 字段 | 类型 | 含义 |
| --- | --- | --- |
| `accountCode` | String | 云账号编号 |
| `monthlyBudget` | double | 月度预算 |
| `actualSpend` | double | 本月实际费用 |
| `previousSpend` | double | 上月费用 |
| `idleResources` | int | 闲置资源数 |
| `untaggedCost` | double | 未标记成本 |
